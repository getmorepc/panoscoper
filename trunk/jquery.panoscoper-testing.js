...
/* Initialize for each instance */
			...
			/******* The panoviewer is a wrapper for future features (multiple layering) *******/
				...
				panoviewer.css('height', $(this).outerHeight()+'px')
				.css('width', '100%')
				.css('display', 'block')
				.css('margin-top', '0')
				.css('overflow', 'hidden');
			...
			/******* Localize the left/right and up/down movement and animation to support multiple panoscopers *******/
			...
			/******* This event is for looking up and down (not implemented yet) *******/
				...
				panoframe.mousemove(function(e){
					...
					if( pano.height() > setPano.height ) {
						ymov = -e.pageY+pagepos.top;
						ymov = ymov*pano.outerHeight();
					}
				});
...
/* Main update function */
			...
			/******* This is where all the magic happens, applies the new left margin to the panobox and updates the up/down motion *******/
					
					panoviewer.css('marginTop', ymov+'px');
			...
			/******* Function to update the Compass and Debug Panel*******/
				...
				function updateCompass(marg){
					...
					if (setPano.debug){
						panodebug.html('Current Bearing: '+(-marg)+'<br/>North: '+setCompass.bearing+'<br/>ymov:'+ymov);
					}
				}
